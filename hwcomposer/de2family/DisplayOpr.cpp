...
#define SCALEMIN 16
#ifdef UI_WITDH
#define MAXUIWIDTH UI_WITDH
#else
#define MAXUIWIDTH 1280
#endif
#ifdef UI_HEIGHT
#define MAXUIHEIGHT UI_HEIGHT
#else
...
typedef enum PipeType {
	NOTASSIGN = 0,
	VIDEO,// video channel
	UI_WITH_ALPHA,//ui channel
	UI_NO_ALPHA,// ui but assigned to vi
} PipeType_t;
...
int dispFd;
int tdFd;
unsigned long deFreq = 254000000;
...
int ChipVer = -1;
/*
   fun: get version of chip
   return 0 :A version, default
   return 1 :B version
 */
int getChipVer() {
	char buf[17] = {0};
	...
	if (fd < 0){
		ALOGD("%s: no soc inf node found", __func__);
		ChipVer = 0;
		return ChipVer;
	}
	ChipVer = ioctl(fd, CHECK_SOC_VERSION, buf);
	if (ChipVer  < 0) {
		ALOGD("%s: no soc inf driver failed", __func__);
		ChipVer = 0;
		return ChipVer;
	}
	ChipVer &= 0xf;
	ALOGD("%s: chipver=0x%x", __func__, ChipVer);
	return ChipVer;
}
#endif
...
void calcGlobleLayerFactor(Display_t *display, Layer_t *layer, float *width, float *hight)
{
	...
	screenW = config->VarDisplayWidth;
	screenH =  config->VarDisplayHeight;
	screenW /= config->width;
	screenH /= config->height;
	calcLayerFactor(layer, &layerW, &layerH);

	*width = screenW * layerW * config->hpercent / 100;
	*hight =  screenH * layerH * config->vpercent / 100;

}

bool displayFrameIsScale(Display_t *display, Layer_t *layer)
{
	float xscale, yscale;
	calcGlobleLayerFactor(display, layer, &xscale, &yscale);

	return !checkFloatSame(xscale, 1.0) || !checkFloatSame(yscale, 1.0);
}

bool layerCanScale(unsigned long deFreq, Display_t *display, Layer_t *layer)
{
	DisplayConfig *config;
	...
	/* the scale smallest ratio is 16,biggest is 32 */
	if ((out_w * out_h * SCALEMIN <= srcW * srcH) ||
			(out_w * out_h >= srcW * srcH * SCALEMAX)) {
		return 0;
	}
	if ((abs(widthScale * hightScale * SCALEMIN) <= 1) ||
			(abs(widthScale * hightScale) >= SCALEMAX)) {
		return 0;
	}
	/*smallest input size is 8x4*/
	if (srcW < 8 || srcH < 4) {
		return 0;
	}
	/*smallest output size is 8x4*/
	if (srcW * widthScale  < 8 || srcH * hightScale < 4) {
		return 0;
	}
	...
	if (deFreq < 1000) {
		return 0;//defreq is too small and deFreq/1000 = 0, div =0 err.
	}

	layerLinePeroid = (srcW > dstW) ? (1000000 * ((long long)(screenW - dstW + srcW))
			/ (deFreq/1000)) : (1000000 * ((long long)screenW) / (deFreq / 1000));

	/*reserve*/
	/*
	layerLinePeroid = (srcW > dstW) ? (1000000*((long long)(srcW)) / (deFreq / 1000)
			* ((srcH > dstH) ? srcH / dstH : 1)) : (1000000 * ((long long)dstW)
			/ (deFreq / 1000)) + 1000000 * ((long long)screenW - dstW)
			/ (deFreq / 1000);
	*/
	/*end*/

	if(((screenLinePeroid * 4 / 5) < layerLinePeroid)
			|| /*avoid unknown-error(flicker) when de scale */
			(((srcW > dstW) && ((srcW * 9 / 10) < dstW))
			&& (sizeList(display->layerSortedByZorder) >= 5))) {
		return 0; //can't
	} else if (!layerIsVideo(layer)) {
...
static int hwc_ui_scaler_check(Display_t *display, Layer_t *layer)
{
	...
	out_w = out_w * dispconfig->hpercent * dispconfig->VarDisplayWidth  / InitDisplayWidth  / 100;
	out_h = out_h * dispconfig->vpercent * dispconfig->VarDisplayHeight / InitDisplayHeight / 100;

	out_w = out_w == 0 ? 1 : out_w;
	out_h = out_h == 0 ? 1 : out_h;

	return de_scale_capability_detect(src_w, src_h, out_w, out_h,
					dispconfig->VarDisplayWidth,
					dispconfig->VarDisplayHeight, 60, 696000000);
}
...
{
	int i = 0, j = 0, cutVi = 0, cutUIAlpha = 0, cutuiButVi = 0;
	Layer_t *layer;
	...
	for (i = from; i <= to; i++) {
		if (hw->Pipe[i].type == VIDEO)
			cutVi++;
		if (hw->Pipe[i].type == UI_WITH_ALPHA)
			cutUIAlpha++;
		if (hw->Pipe[i].type == UI_NO_ALPHA)
			cutuiButVi++;
	...
	if (from == 0) {
		...
		hw->mustViPipe -= cutVi;
		hw->mustUIPipe -= cutUIAlpha;
		hw->uiButVi -= cutuiButVi;
...
static bool resize_layer(LayerSubmit_t *submitLayer, disp_layer_info2 *layer_info , Layer_t *layer)
{
	hwc_rect_t sourceCrop;
	hwc_rect_t displayFrame;
	DisplayConfig_t *dispconfig;

	dispconfig = &submitLayer->currentConfig;

	int VarDisplayWidth = dispconfig->VarDisplayWidth;
	int VarDisplayHeight = dispconfig->VarDisplayHeight;
	int InitDisplayWidth = dispconfig->width;
	int InitDisplayHeight = dispconfig->height;
	...
	if(layerIsVideo(layer)) {
		...
		recomputeDisplayFrame(dispconfig->screenRadio, sourceCrop,
				unitedFrameWidth, unitedFrameHeight, &displayFrame);
	}

	layer_info->screen_win.x =
		calc_point_byPercent(dispconfig->hpercent, InitDisplayWidth >> 1, displayFrame.left)
		* VarDisplayWidth / InitDisplayWidth;
	layer_info->screen_win.width =
		calc_point_byPercent(dispconfig->hpercent, InitDisplayWidth >> 1, displayFrame.right)
		* VarDisplayWidth / InitDisplayWidth;
	...
	layer_info->screen_win.y =
		calc_point_byPercent(dispconfig->vpercent, InitDisplayHeight >> 1, displayFrame.top)
		* VarDisplayHeight / InitDisplayHeight;
	layer_info->screen_win.height =
		calc_point_byPercent(dispconfig->vpercent, InitDisplayHeight >> 1, displayFrame.bottom)
		* VarDisplayHeight / InitDisplayHeight;
...
void resetDisplayConfig(Display_t *display)
{
	...
	pthread_mutex_lock(&display->ConfigMutex);
	dispconfig->vpercent = display->vpercent;
	dispconfig->hpercent = display->hpercent;
	dispconfig->VarDisplayHeight = display->VarDisplayHeight;
	dispconfig->VarDisplayWidth = display->VarDisplayWidth;
	dispconfig->screenRadio = display->screenRadio;
	pthread_mutex_unlock(&display->ConfigMutex);
}
...
bool inline checkSupportTransform(Display_t *display, Layer_t *layer)
{
	...
	return supportTR(display, layer) && aquireTRlimmit(layer);
}
...
bool checkSimpleSupport(Display_t *display, Layer_t *layer)
{
...
#if (TARGET_BOARD_PLATFORM == cupid)
	/* only A version of chip*/
	if (getChipVer() == 0) {
#ifdef ENABLE_WRITEBACK
		if (display->displayId == 1 && !isNeedWb(layer)) {
	...
	if (!checkSoildLayer(layer) && ((layer->crop.right - layer->crop.left < 1)
			|| (layer->crop.bottom - layer->crop.top < 1))) {
		layer->duetoFlag = FORCE_GPU;
		return false;

	}
	if (layerIsVideo(layer) && layerIsPremult(layer)){
		private_handle_t *hdl = (private_handle_t *)layer->buffer;
		if (hdl && is_afbc_buf(hdl)) {
			//layer->blendMode = HWC2_BLEND_MODE_NONE;
		} else {
			layer->duetoFlag = VIDEO_PREM;
			return false;
		}
	}

	if (checkSkipLayer(layer)){
/*gpu of h6 not support afbc, force assign to DE*/
#ifdef  SUN50IW6P1
		private_handle_t *hdl = (private_handle_t *)layer->buffer;
		if (hdl && is_afbc_buf(hdl)) {
			//ALOGD("h6 force afbc layer to de");
		} else {
	...
	if (displayFrameIsScale(display, layer) && !layerCanScale(deFreq, display, layer)) {
...
static bool canAssignSpecialPipe(Display_t *display, PipeInfo_t *matchPipe, 
			Layer_t *layer, PipeType_t type)
{
	DESource_t *deHw = &DESource[display->displayId];
	unusedpara(matchPipe);

	if (layerIsVideo(layer)) {
		if (type == VIDEO && deHw->mustViPipe < deHw->fixVideoPipeNum) {
			return true;
		}
	} else {
		if (type == UI_WITH_ALPHA) {
			bool add = display->needclientTarget;
			if (layer->compositionType == HWC2_COMPOSITION_CLIENT_TARGET)
				add = 0;
			if (add + deHw->mustUIPipe
...
void reAssignHwPipeZorder(DESource_t *hw)
{
	...
	int i = 0, j = 0, z = 0, zOrder = 0, uiBigen = hw->fixVideoPipeNum, viBigen = 0, pipe = 0;
	Layer_t *layer = NULL, *FBlayer = NULL;
	DELayerPrivate_t *hwlayer = NULL;
	...
	for (i = 0; i < hw->fixPipeNumber; i++) {
		if (hw->Pipe[i].type == NOTASSIGN)
			continue;
		/* the z=0 chanel must no pixel alpha */
		hwPipe = &hw->Pipe[i];
		if (hw->Pipe[i].type == UI_WITH_ALPHA) {
			pipe = uiBigen++;
			...
			if (hw->mustViPipe >= hw->fixVideoPipeNum) {
				pipe = uiBigen++;
			} else {
				pipe = viBigen++;
				hw->mustViPipe++;
...
void setPipeScale(PipeInfo_t *Pipe, Layer_t *layer)
{
	...
	if (!checkFloatSame(Pipe->scaleW, 1.0)
		|| !checkFloatSame(Pipe->scaleH, 1.0))
		return;
	...
	Pipe->scaleH = (layer->frame.bottom - layer->frame.top) / h;
}

int matchPipeAttribute(Display_t *display, int i, Layer_t *uplayer)
{
	DESource_t *deHw = &DESource[display->displayId];
	...
	if (Pipe->layNum == 0) {
		if (layerIsVideo(uplayer)) {
			if (deHw->mustViPipe < deHw->fixVideoPipeNum)
				return 0;
			ret = -3;
		}else{
			if (layerIsPixelBlended(uplayer)) {
				bool add = display->needclientTarget;
				if (uplayer->compositionType == HWC2_COMPOSITION_CLIENT_TARGET)
					add = 0;
				if ((deHw->mustUIPipe + add)
						< (deHw->fixPipeNumber - deHw->fixVideoPipeNum))
					return 0;
				ret = -4;
			}else{
	...
	if (layerIsVideo(uplayer)) {
		...
		if (layerIsPixelBlended(uplayer)) {
			...
			if (Pipe->type == UI_NO_ALPHA
				&& !canAssignSpecialPipe(display, Pipe, uplayer, UI_WITH_ALPHA)) {
				ret = -10;
				goto not_match;
			}
		}
	...
	return 0;
not_match:
	ALOGV("####channel %d not match ret = %d####", i, ret);
	return ret;
}

Layer_t *reCalPipeForVideoLayer(DESource_t *deHw, PipeInfo_t *Pipe, Layer_t *videoLayer)
{
	Layer_t *layer;
	int i = 0;
	if (deHw->uiButVi) {
		for (i = 0; i < deHw->fixPipeNumber; i++) {
			if (deHw->Pipe[i].type == UI_NO_ALPHA) {
				deHw->Pipe[i].type = UI_WITH_ALPHA;
				deHw->uiButVi--;
				break;
			}
		}
...
Layer_t* assignLayersToPipe(Display_t *display, Layer_t *layer)
{
	...
	for (i = 0; i < deHw->fixPipeNumber; i++) {
		if (matchPipe == NULL) {
			 if(!matchPipeAttribute(display, i, layer)) {
				matchPipe = &deHw->Pipe[i];
				matchId = i;
				if (matchPipe->layNum == 0) {
					break;
				}
				if (layerIsPixelBlended(layer)
	...
	ALOGV("layer:%d top i:%d match pipe:%d %p ui:%d vi:%d ui2vi:%d current:%d",
		layer->zorder, i, matchId, matchPipe, deHw->mustUIPipe, deHw->mustViPipe, deHw->uiButVi, deHw->currentPipe);
	if (matchPipe == NULL) {
		if (layerIsVideo(layer)) {
			if (canAssignSpecialPipe(display, NULL, layer, VIDEO)) {
				...
				assignedlayer = reCalPipeForVideoLayer(deHw, &deHw->Pipe[resetedPipe], layer);
...
assign_FB:
	...
	if (matchPipe) {
		...
		if (matchPipe->type == UI_NO_ALPHA || matchPipe->type == NOTASSIGN) {
			if (layerIsVideo(layer)) {
				matchPipe->type = VIDEO;
				deHw->mustViPipe++;
			} else{
				/* preferential use VI channel , so if can assgin it to VIchannel , assign it */
				if (!layerIsPixelBlended(layer)
					&& canAssignSpecialPipe(display, matchPipe, layer, VIDEO)) {
					if (matchPipe->type == NOTASSIGN)
						deHw->uiButVi++;
					matchPipe->type = UI_NO_ALPHA;
				}else{
				/* if no alpha ui, but there is not VI to use, so must UI_WITH_ALPHA mustUIPipe++
				  * more UI without alpha
				  */
					deHw->mustUIPipe++;
					if (matchPipe->type == UI_NO_ALPHA)
						deHw->uiButVi--;
					matchPipe->type = UI_WITH_ALPHA;
				}
			}
		}
		if (matchPipe->layNum == 0)
...
static void setup3dMode(int hwid, disp_layer_info2 *info)
{
	if (hwid < 0) {
		/* not support. */
		return;
...
static void setup3dUILayer(int hwid, disp_layer_info2 *info)
{
    if (hwid < 0) {
        /* not support. */
        return;
...
int setupSolidColorLayer(LayerSubmit_t *submitLayer, Layer_t *layer, disp_layer_config2 *layerConfig)
{
	...
	resize_layer(submitLayer, info , layer);
	return 0;
...
#else
	unusedpara(layer_info);
	unusedpara(dataspace);
#endif
	return 0;
}

static int setupDisplayInfo(LayerSubmit_t *submitLayer, Layer_t *layer, disp_layer_config2 *layerConfig, int zorder)
{
	...
	if(layer->buffer == NULL) {
		setupSolidColorLayer(submitLayer, layer, layerConfig);
		return 0;
	}
	...
	if (layerIsVideo(layer)) {
		...
		// setup snr info for DE0::vchannel0
		DELayerPrivate_t* hwlayer = toHwLayer(layer);
		if ((submitLayer->hwid == 0) && (hwlayer->pipe == 0)) {
			setup_snr_info(&info->snr);
		}
	}

	setup_layer_dataspace(info, layer->dataspace);

	if(submitLayer->currentConfig.mode == DISP_TV_MOD_1080P_24HZ_3D_FP) {
		if (layerIsVideo(layer)) {
			setup3dMode(submitLayer->hwid, info);
		} else {
			//info->b_trd_out = 0;
			//info->fb.flags = DISP_BF_STEREO_2D_DEPTH;
			//info->fb.depth = 3;
			setup3dUILayer(submitLayer->hwid, info);
		}
	}
...
int de2memCtrlAddLayer(Display_t *display, Layer_t *layer)
{
	...
	if (layer->transform & HAL_TRANSFORM_ROT_90) {
		withS = (layer->crop.bottom - layer->crop.top)/(layer->frame.right - layer->frame.left);
		hightS = (layer->crop.right- layer->crop.left)/(layer->frame.bottom - layer->frame.top);
	}else{
		withS = (layer->crop.right - layer->crop.left)/(layer->frame.right - layer->frame.left);
		hightS = (layer->crop.bottom - layer->crop.top)/(layer->frame.bottom - layer->frame.top);
	}
...
int de2SetupLayer(LayerSubmit_t *submitLayer)
{
	struct listnode *node;
	DESource_t *hw;
	Layer_t *layer;
	...
	DELayerPrivate_t *hwlayer;
	int i = 0;
	int chn = submitLayer->hwid? 2 : 4;

	hw = &DESource[submitLayer->hwid];
	...
	list_for_each(node, &submitLayer->layerNode) {
		...
		setupDisplayInfo(submitLayer, layer,
...
static int diplayToScreen(int disp, disp_layer_config2 *config, int num, unsigned int synccount)
{
	...
	{
		ATRACE_NAME("DISP_LAYER_SET_CONFIG2");
...
static int de2commitToDisplay(Display_t *display, LayerSubmit_t *submitLayer)
{
	...
	hw = &DESource[submitLayer->hwid];
	layerConfig = hw->layerInfo;
	if (display->displayId != submitLayer->hwid)
		return 0;
	return diplayToScreen(submitLayer->hwid, layerConfig, pipe * 4, submitLayer->sync.count);
...
extern const char *hwcPrintInfo(enum sunnxi_dueto_flags eError);
void showlayers(Display_t *display)
{
	...
	ALOGI("dispy%d(hwid%d) cur:%d-%d-%d mem:%s\n", (int)toClientId(display->clientId), display->displayId, display->frameCount,
...
void de2Dump(Display_t *display, uint32_t* outSize, char* outBuffer, uint32_t max)
{
	...
	count += sprintf(outBuffer + count, "disp:%d cur:%u-%u ker:%u\n"
			"--------------------------------------------------------------------------"
			"--------------------------------------------------------------------------\n",
...
static int setActiveConfig(Display_t *display, DisplayConfig_t *config)
{
	...
	arg[2] = config->mode;
	hwdisplay->type.mode = config->mode;

	ALOGD("%s:display:%d  type: %d. mode: %d",
			__FUNCTION__, display->displayId, hwdisplay->type.type, config->mode);
	if (hwdisplay->type.type == DISP_OUTPUT_TYPE_LCD)
		return 0;

#ifndef TARGET_PLATFORM_HOMLET

	if (hwdisplay->type.type == DISP_OUTPUT_TYPE_HDMI) {
		struct disp_device_config conf;
		/*arg[0] = display->displayId;
		arg[1] = (unsigned long)&conf;
		if (ioctl(dispFd, DISP_DEVICE_GET_CONFIG, (void*)arg) < 0) {
			ALOGE("getDeviceConfig failed!");
			return -1;
		}*/

		conf.type = (enum disp_output_type)hwdisplay->type.type;
		...
		arg[0] = display->displayId;
		arg[1] = (unsigned long)&conf;
		if (ioctl(dispFd, DISP_DEVICE_SET_CONFIG, (void*)arg) < 0) {
			ALOGE("setDeviceConfig failed!");
			return -1;
		}
	} else {
...
int switchDisplay(Display_t *display, int type, int mode)
{
	...
	for (i = 0; i < display->configNumber; i++) {
		hwconfig = display->displayConfigList[i];
...
int initPermanentDisplay(Display_t *display)
{
	...
	config = (DisplayConfig_t*)hwc_malloc(sizeof(DisplayConfig_t));
	if (config == NULL){
	...
	config->VarDisplayWidth = config->width;
	config->VarDisplayHeight = config->height;
...
loop:
...
#else
		displayconfig->VarDisplayWidth = displayconfig->width;
		displayconfig->VarDisplayHeight = displayconfig->height;
#endif
	...
	ALOGD("disp:%d active:%d  mode:%d [%d x %d] maxid:%d", display->displayId, fix,
			display->displayConfigList[fix]->mode,
			display->displayConfigList[fix]->width,
			display->displayConfigList[fix]->height, numbconfig);
	...
	if (toClientId(display->clientId) != 0) {
		...
		if (!hwc_getPriDispSlt(&w, &h)) {
			/*too large ui bring scale out or out of ddr bandwidth*/
			if (w  >  MAXUIWIDTH || h > MAXUIHEIGHT) {
				displayconfig->width = MAXUIWIDTH;
				displayconfig->height = MAXUIHEIGHT;
			} else {
				displayconfig->width = w;
				displayconfig->height = h;
			}
		}
	}
...
int initVariableCvbs(Display_t *display)
{
	...
	hwdisplay = toHwDisplay(display);
	tv_mode = hwdisplay->type.mode;

	display->displayConfigList = (DisplayConfig_t **)hwc_malloc(sizeof(DisplayConfig_t *));
	if (display->displayConfigList == NULL) {
		ALOGE("%s malloc err...", __func__);
		return -1;
	}
	displayconfig = (DisplayConfig_t *)hwc_malloc(sizeof(DisplayConfig_t));
...
int init_sync(int hwid)
{
	...
	ALOGD("init a new de %d sync, deFreq=%lu", hwid, deFreq);
	return 0;
}

int deinit_sync(int hwid)
{
	unsigned long arg[4] = {0};
	...
	if (ioctl(dispFd, DISP_HWC_COMMIT, (unsigned long)arg)) {
		ALOGE("destroy a timeline %d !!!", hwid);
		return -1;
	}
	ALOGD("destroyed a de %d sync.", hwid);
	return 0;
}
...
int de2Init(Display_t* display)
{
	...
	if (display->plugIn == 1 && display->displayId == 0) {
	...
	do {
		...
		if (tryTime > 50 && display->displayId == 0) {
			...
			hwdisplay->type.mode = display->default_mode;
			arg[0] = display->displayId;
			arg[1] = hwdisplay->type.type;
...
#endif
#if (TARGET_BOARD_PLATFORM == t3)
#ifndef TARGET_PLATFORM_HOMLET
		if (display->displayId == 1 && tryTime >= 1 ) {
			if (cvbsfd > 0) {
	...
	if (tryTime > 0) {
		ALOGD("disp %d type = none:%d ms", display->displayId, tryTime * 20);
	}
	if (display->displayId == 1) {
		arg[0] = 0;
...
int de2Deinit(Display_t *display)
{
	...
	clearList(display->layerSortedByZorder, 1);
	pthread_mutex_unlock(&display->listMutex);
/* JetCui: becareful for switch displayid */
	deinit_sync(display->displayId);
...
int32_t de2SetPowerMode(Display_t* display, int32_t mode)
{
	...
	// Display driver only response for primary display's runtime suspend request,
	// So force to set the display id as '0' here!
	arg[0] = 0;
	...
	switch (mode) {
		...
		case HWC2_POWER_MODE_DOZE_SUSPEND:
			// Not support doze mode yet.
			return HWC2_ERROR_UNSUPPORTED;
		case HWC2_POWER_MODE_ON:
			arg[1] = 0;
			// Do not override the vsync enable config from SF!
			// Or, we will get fail in VTS.
			vsyncEn = display->vsyncEn;
			break;
		default:
...
int32_t de2SetVsyncEnabled(Display_t *display, int32_t enabled)
{
	...
	display->vsyncEn = (enabled == HWC2_VSYNC_ENABLE ? 1 : 0);
...
int displayDeviceInit(Display_t ***display)
{
	...
	for (i = 0; i < DE_NUM; i++) {
		...
		if (ret > 0)
			...
			ALOGD("get [Disp%d] output type is %d!\n", i, hwdisplay->type.type);
		...
		disp->layerSortedByZorder = (struct listnode*)hwc_malloc(sizeof(struct listnode));
		list_init(disp->layerSortedByZorder);
		pthread_mutex_init(&disp->listMutex, 0);
		pthread_mutex_init(&disp->ConfigMutex, 0);
